...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        qVal = 0
        for (nextState, currProb) in self.mdp.getTransitionStatesAndProbs(state, action):
            currReward = self.mdp.getReward(state, action, nextState)
            currValue = self.values[nextState]
            qVal += currProb * (currReward + currValue * self.discount)
        return qVal
