        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if not hitsWall:
                if (nextx, nexty) in self.corners and (nextx, nexty) not in visitedCorners:
                    successorState = ((nextx, nexty), visitedCorners + ((nextx, nexty), ))
                else:
...
def cornersHeuristic(state, problem):
    ...
    while len(visitedCorners) < 4:
        parentHeuristic = 100000
        parentState = None
        for corner in corners:
            if corner not in visitedCorners:
                h = util.manhattanDistance(currentPosition, corner)
                if h < parentHeuristic:
                    parentHeuristic = h
                    parentState = corner
        cornerHeuristic += parentHeuristic
        currentPosition = parentState
        visitedCorners = visitedCorners + ((parentState), )
...
def foodHeuristic(state, problem):
    ...
    for food1 in foodGrid.asList():
        listFoodToOtherFood = []
        distFromPacmanToFood = util.manhattanDistance(position, food1)
        for food2 in foodGrid.asList():
            listFoodToOtherFood.append(util.manhattanDistance(food1, food2))
        maxFoodDist = max(listFoodToOtherFood)
        listPacmanToFood.append(distFromPacmanToFood + maxFoodDist)
    return min(listPacmanToFood)
