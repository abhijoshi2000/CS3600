...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        currWeightedSum = self.getWeightedSum(wOne)
        return self.sigmoid(currWeightedSum)
    ...
    def sigmoidDeriv(self, value):
        ...
        """
        """YOUR CODE"""
        return exp(value) / ((exp(value) + 1) * (exp(value) + 1) * 1.0)
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for percept in layer:
                percepts.append(percept.sigmoidActivation(prev))
            out.append(percepts)
            prev = percepts
        return out
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    error = 0.0
                    for next in xrange(len(nextLayer)):
                        delta = nextLayer[next].weights[neuronNum + 1] * deltas[0][next]
                        error += delta
                    delta = gPrime * error
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for exIn, exOut in examplesTest:
        out = nnet.feedForward(exIn)[-1]
        res = True
        for i in range(len(out)):
            if round(out[i]) != exOut[i]:
                res = False
        if res:
            testCorrect += 1
        else:
