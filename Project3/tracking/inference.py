...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in positions:
            gameStateOne = self.setGhostPosition(gameState, p)
            distOne = self.getPositionDistribution(gameStateOne)
            for position, probability in distOne.items():
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        particles = []
        ct = 0
        while ct < self.numParticles:
            positions = self.legalPositions
            particles.append(positions[ct % len(positions)])
            ct+=1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            jailParticles = []
            for num in range(0, numberOfParticles):
                jailParticles.append(currentJailPosition)
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.particles:
            newPosDistance = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
            newParts.append(util.sample(newPosDistance))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        configs = list(itertools.product(legalPositions, repeat=self.numGhosts))
        ...
        for particle in particles:
            ...
            for i in range(numGhosts):
                if noisyDistances[i] != None:
                    ghostPositions = particle[i]
                    d = abs(ghostPositions[0] - pacmanPosition[0]) + abs(ghostPositions[1] - pacmanPosition[1])
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            numGhosts = self.numGhosts
            ghostAgents = self.ghostAgents

            for i in range(numGhosts):
                dist = getPositionDistributionForGhost(
                    setGhostPositions(gameState, newParticle), i, ghostAgents[i])
